...
FIXED VERSION with correct S3 metrics path
"""
import os
import json
import time
...
def get_metrics_from_s3(model_artifacts_path, bucket_name):
    ...
    try:
        ...
        # Metrics are in the same directory as model.tar.gz
        metrics_key = f"{key_prefix}/metrics.json"
        
        print(f"Model artifacts path: {model_artifacts_path}")
        print(f"Looking for metrics at: s3://{bucket}/{metrics_key}")
        ...
        print(f"✗ Metrics file not found at s3://{bucket}/{metrics_key}")
